...
class PM400:
    def __init__(self, device_sn:str="") -> None:
        ...
        self.logger.info(f"Initiating bsl_instrument - PM400({device_sn})...")
        if self._com_connect(device_sn):
            self.logger.device_id = self.device_id
            self.run_update_power_meter()
            self.logger.success(f"READY - Thorlab PM400 Power Meter \"{self.device_id}\" with sensor \"{self.get_sensor_id()}\".\n\n\n")
        else:
            self.logger.error(f"FAILED to connect to Thorlab PM400 ({device_sn}) Power Meter!\n\n\n")
