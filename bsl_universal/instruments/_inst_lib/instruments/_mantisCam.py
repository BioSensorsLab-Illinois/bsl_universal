...
class MantisCamCtrl:
    ...
    class FILE_SAVING_MODE(enum.Enum):
        CUSTOM = "Custom"; TIMESTAMP = "Timestamp"
    ...
    def __init__(self, device_sn="", port_cmd_pub: int=60000, port_cmd_sub: int=60001, port_vid_sub: int=60011, *, log_level: str = "TRACE", is_GSENSE: bool = False):
        ...
        self.__zmq_init()
        time.sleep(1)
        logger.success(f"Camera - ZMQ interface connected, camera initialized!")
    ...
    def __zmq_term(self):
        ...
        self.__closed = True
        try:
            self.poller.unregister(self.cmd.skt_sub)
        except Exception:
            pass
        try:
    ...
    def __logger_init(self):
        ...
        log_dir = os.path.join(home_dir, 'MantisCam', 'AutoRecord', 'logs')
        os.makedirs(log_dir, exist_ok=True)
        self.__log_file_id = logger.add(
            os.path.join(log_dir, datetime.now(timezone.utc).strftime('%Y-%m-%d_%H-%M-%S_%f')[:-3]+'.log'),
        ...
        self.__log_console_id = logger.add(
            sys.stdout,
    ...
    def get_frame_mean_gs_hg(self, timeout_ms: int = 5000) -> float:
        ...
        """
        return self.get_frame_mean_name('High Gain', timeout_ms=timeout_ms)


    def get_frame_mean_gs_lg(self, timeout_ms: int = 5000) -> float:
        """
        - Get the mean value of the Low Gain frame.
        ...
        -------
        mean : `float`
            Mean value of the Low Gain frame.
        """
        return self.get_frame_mean_name('Low Gain', timeout_ms=timeout_ms)


    def get_frame_mean_name(self, frame_name: str="High Gain", sub_frame_type: str='', timeout_ms: int = 5000) -> float:
        """
        - Get the mean value of the specified frame.
        ...
        -------
        mean : `float`
            Mean value of the requested frame.
        """
    ...
    def set_exposure_ms(self, exp_time_ms: float = 15, raise_error: bool = False) -> None:
        ...
        raise_error : `bool`
            (default to False)
            If True, raises DeviceTimeOutError on timeout instead of retrying.
        """
        ...
        while True:
            ...
            if self.__e_exp_matched:
                ...
                return
            logger.trace(f"Camera - waiting for exposure match, desired {exp_time_ms}ms.")
            if time.time() - time_start > self.TIMEOUT_SEC:
                if raise_error:
                    logger.error(f"ERROR - Camera exposure time not match, timed out!")
                    raise bsl_type.DeviceTimeOutError
                logger.warning(f"Camera - exposure timeout, retrying with {exp_time_ms}ms...")
    ...
    def run_auto_exposure(self, frame_name: str='High Gain', sub_frame_type: str='', run_rgb_max_chan: bool=False, min_exp_ms=1, max_exp_ms=2500, target_mean=30000, max_iter=10, hysteresis=2000):
        """
        ...
        frame_name : `str`
            (default to 'High Gain')
            Frame name to run auto exposure on.
            Options: 'High Gain', 'Low Gain', etc.
        ...
        hysteresis : `float`
            (default to 2000)
            Hysteresis for the auto exposure to reach the target mean value.

        Returns
        -------
        result : `float`
            The final exposure time in milliseconds.
        """
    ...
    def set_folder_name(self, create_new_folder: bool=True, time_stamp_only: bool=False, folder_name: str="video"):
        """
        - Set the folder name for recording files.
    ...
    def set_start_record(self, n_frames: int = 100, wait_until_done: bool = True):
        ...
        if wait_until_done:
            time_start = time.time()
            timeout_limit = (self.__e_exp_time * n_frames)/500 + 20
            while True:
                self.__zmq_recv()
    ...
    def close(self) -> None:
        """
        - Close the MantisCam connection and release all resources.
        """
        self.__zmq_term()
        self.__logger_term()
        logger.info("MantisCam connection closed.")
    ...
    def __logger_term(self):
        ...
        logger.complete()
        try:
            logger.remove(self.__log_file_id)
        except Exception:
            pass
        try:
            logger.remove(self.__log_console_id)
        except Exception:
            pass
