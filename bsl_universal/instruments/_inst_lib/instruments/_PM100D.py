...
class PM100D:
    ...
    def __init__(self, device_sn:str="") -> None:
        """
        Initialize and connect a PM100D power meter.
    ...
    def _com_connect(self, device_sn:str) -> bool:
        """
        Connect to PM100D with bounded retries.
        ...
        for attempt in range(1, self.CONNECT_RETRY_COUNT + 1):
            try:
                candidate = bsl_visa(inst.PM100D, device_sn)
        ...
        self.logger.error(f"Unable to connect PM100D after retries: {repr(last_error)}")
        return False

    def reconnect(self, device_sn: str = "") -> bool:
        """
        Reconnect to the PM100D controller.
    ...
    def reset_meter(self, run_update: bool = True) -> bool:
        """
        Reset PM100D and optionally refresh cached readbacks.
        ...
        try:
            ...
            self.logger.error(f"Failed to reset PM100D: {type(exc)}")
            return False
    ...
    def close(self) -> None:
        """
        Close PM100D communication resources safely.
        """
